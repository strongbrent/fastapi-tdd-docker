...
def test_update_summary(test_app_with_db):
    ...
    response = test_app_with_db.put(
        f"/summaries/{summary_id}/", data=json.dumps({"url": "https://foo.bar", "summary": "updated!"})
    )
    assert response.status_code == 200
...
def test_update_summary_incorrect_id(test_app_with_db):
    ...
    response = test_app_with_db.put("/summaries/0/", data=json.dumps({"url": "https://foo.bar", "summary": "updated!"}))
    assert response.status_code == 422
    assert response.json() == {
        "detail": [
            {
...
def test_update_summary_invalid_keys(test_app_with_db):
    ...
    response = test_app_with_db.put(
        f"/summaries/{summary_id}/", data=json.dumps({"url": "invalid://url", "summary": "updated!"})
    )
    assert response.status_code == 422
    assert response.json()["detail"][0]["msg"] == "URL scheme not permitted"
